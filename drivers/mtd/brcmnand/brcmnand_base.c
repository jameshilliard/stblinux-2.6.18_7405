...
#ifdef HUMAX_GENKERNEL
static int blockmap_for_gang[NAND_BLOCKNUM]; // For Gang
static int blockmap_for_fusing[NAND_BLOCKNUM]; // For Download

static int blockmap_for_gang_enable = 0;
static int blockmap_for_fusing_enable = 0;
#endif
...
static int nand_get_block_status(uint32_t blockbase, int blk_size, int pg_size);
#endif
...
if (gdebug) printk("CS=%d, chip->CS[cs]=%d\n", cs, chip->CS[cs]);
	...
	#ifdef HUMAX_GENKERNEL
	if((ldw >= BMTforGang_SIZE) && (ldw < RESERVED_AREA_OFFSET))
		{
			pagenum		= ldw%(BLOCK_SIZE_128K);
			blocknum	= ldw/(BLOCK_SIZE_128K);

			//block address remapping (Download)
		if(blockmap_for_fusing_enable)
			blocknum	= blockmap_for_fusing[blocknum];

			//block address remapping (Gang)
		if(blockmap_for_gang_enable)
			blocknum	= blockmap_for_gang[blocknum];
...
#ifdef HUMAX_GENKERNEL	//BMT of NAND Flash
/**************************************************************************
	BMT Information
***************************************************************************
	Block 0 : Bad block map table for gang.
	Block 1 : Bad block map table for fursing.
	Block 2 : Bad block map table for gursing (backup).
**************************************************************************/
...
#define BMT_TABLE_SIZE						(2*NAND_BLOCKNUM)	//2*2048
#define BMT_CRC_SIZE						2

#define BMT_TABLE_OFFSET_INVALID			0xFFFFFFFF

#define NAND_ERROR_READ_DISTURB				(1)
#define NAND_ERROR_READ_DISTURB_WITH_UCE	(-1)

#define NAND_PAGE_SIZE					  	2048	/* Bytes */
#define NAND_INTERNAL_PAGE_BUFFER_SIZE  	512		/* Bytes */

#define PHYS_TO_K1(x)		((x) | 0xa0000000)

#define REG(ADDR)			(*((volatile uint32_t*)(PHYS_TO_K1(BCHP_PHYSICAL_OFFSET+ADDR))))
#define REG_MASK(c,r,f)		c##_##r##_##f##_MASK
#define REG_SHIFT(c,r,f)	c##_##r##_##f##_SHIFT

#define GET_REG_FIELD(m,c,r,f)		((((m)= ((m) & REG_MASK(c,r,f)) >> REG_SHIFT(c,r,f))))
#define SET_REG_FIELD(m,c,r,f,d)	((m) = (((m) & ~REG_MASK(c,r,f)) | ((((d) ) << REG_SHIFT(c,r,f)) & REG_MASK(c,r,f))))
...
static uint32_t nand_do_cmd(uint32_t cmd, uint32_t address)
{
	uint32_t	status;
	uint32_t	pagenum, blocknum;

	if((address >= BMTforGang_SIZE) && (address < (MIDDLE_OFFSET-RESERVED_BLOCK)))
	{
		pagenum		= address%BLOCK_SIZE_128K;
		blocknum	= address/BLOCK_SIZE_128K;
		...
		address = (uint32_t)(blocknum*BLOCK_SIZE_128K+ pagenum);
	}

	brcmnand_ctrl_write(BCHP_NAND_CMD_ADDRESS, address);
	brcmnand_ctrl_write(BCHP_NAND_CMD_START, cmd << BCHP_NAND_CMD_START_OPCODE_SHIFT);

	for(;;)
	{
		status = brcmnand_ctrl_read(BCHP_NAND_INTFC_STATUS);
		if((status & BCHP_NAND_INTFC_STATUS_CTLR_READY_MASK) != 0)
		{
			break;
		}
	}

	return (brcmnand_ctrl_read(BCHP_NAND_INTFC_STATUS) & 0x1);
}

static int32_t nand_check_read_status(void)
{
	...
	status = REG(BCHP_HIF_INTR2_CPU_STATUS);
	if((status & BCHP_HIF_INTR2_CPU_STATUS_NAND_CORR_INTR_MASK) != 0)
	{
		/* clear the interrupt first */
		temp = REG(BCHP_HIF_INTR2_CPU_CLEAR);
		temp |= BCHP_HIF_INTR2_CPU_CLEAR_NAND_CORR_INTR_MASK;
		REG(BCHP_HIF_INTR2_CPU_CLEAR) = temp;

		rval = 1;
	}

	if((status & BCHP_HIF_INTR2_CPU_STATUS_NAND_UNC_INTR_MASK) != 0)
	{
		temp = REG(BCHP_HIF_INTR2_CPU_CLEAR);
		temp |= BCHP_HIF_INTR2_CPU_CLEAR_NAND_UNC_INTR_MASK;
		REG(BCHP_HIF_INTR2_CPU_CLEAR) = temp;
...
static int32_t nand_write_page (volatile uint8_t* addr, uint8_t* data, uint32_t page_size)
{
	uint32_t internal_nand_buffer_size = NAND_INTERNAL_PAGE_BUFFER_SIZE;

	uint32_t num_buffer_writes;

	uint32_t return_val = 0;
	uint32_t i,j;

	volatile uint32_t*	nand_flash_cache = (volatile uint32_t*)PHYS_TO_K1(BCHP_PHYSICAL_OFFSET + BCHP_NAND_FLASH_CACHEi_ARRAY_BASE );
	uint32_t*			data_buf = (uint32_t*)data;

	num_buffer_writes = page_size/internal_nand_buffer_size;

	return_val = 0;

	for(i = 0; i < num_buffer_writes; i++)
	{
		for(j = 0; j < internal_nand_buffer_size/4; j++)
		{
			nand_flash_cache[j] = data_buf[j + (i * (internal_nand_buffer_size/4))];
		}

		return_val = nand_do_cmd(BCHP_NAND_CMD_START_OPCODE_PROGRAM_PAGE, ((uint32_t)addr + (i * internal_nand_buffer_size)));
		if(return_val != 0)
		{
			break;
		}
...
static int32_t nand_read_page(uint32_t page_address, uint8_t* data, uint32_t offset, uint32_t size)
{
	...
	volatile uint8_t*	nand_flash_cache = (volatile uint8_t*)(PHYS_TO_K1(BCHP_PHYSICAL_OFFSET + BCHP_NAND_FLASH_CACHEi_ARRAY_BASE));

	nand_do_cmd(BCHP_NAND_CMD_START_OPCODE_PAGE_READ, page_address);

	read_status = nand_check_read_status();
	if(read_status == 1)
	{
		printk(KERN_ERR "!!! nand_read_page() - Correctable Error (0x%08X) !!!\n", page_address);
	}
	else if(read_status == -1)
	{
		printk(KERN_ERR "!!! nand_read_page() - un - Correctable Error (0x%08X) !!!\n", page_address);
	}

	for(i = 0; i < size; i++)
	{
		data[i] = nand_flash_cache[offset + i];
	}

	return read_status;
}

static int32_t nand_erase_block(uint32_t blk_addr)
{
	int32_t rval = 0;

	blk_addr = blk_addr & (~(BLOCK_SIZE_128K - 1));

	rval = nand_do_cmd(BCHP_NAND_CMD_START_OPCODE_BLOCK_ERASE, blk_addr);
	if(rval != 0)
	{
		printk(KERN_ERR "nand_erase_block() - block_erase_failed\n");
	}

	return rval;
}

static int nand_read_block(uint32_t blockbase,unsigned char *buf_ptr,int size)
{
	int			i, j, page_num;
	int			nand_intfc_status;
	uint32_t	read_distrb_detect = 0;

	page_num	= size/NAND_INTERNAL_PAGE_BUFFER_SIZE;

	if((size%NAND_INTERNAL_PAGE_BUFFER_SIZE)!=0)
		page_num++;

	if((blockbase%NAND_INTERNAL_PAGE_BUFFER_SIZE)!=0)
		page_num++;

	for(i=0; i < page_num; i++)
	{
		nand_intfc_status = nand_read_page(blockbase+(i*NAND_INTERNAL_PAGE_BUFFER_SIZE),
								buf_ptr+(i*NAND_INTERNAL_PAGE_BUFFER_SIZE), 0, NAND_INTERNAL_PAGE_BUFFER_SIZE);

		if((nand_intfc_status == 1) && (read_distrb_detect == 0))
		{
			// we detected a read distrb
			read_distrb_detect = NAND_ERROR_READ_DISTURB;
		}
		else if(nand_intfc_status == -1)
		{
			read_distrb_detect = NAND_ERROR_READ_DISTURB_WITH_UCE;
		}
	}

	return read_distrb_detect;
}

static int32_t nand_block_write(uint32_t blockbase, uint8_t* data, uint8_t* read_buffer,uint32_t offset, uint32_t page_size, uint32_t copy_len)
{
	uint32_t	i;
	int32_t		return_val = 0;
	uint32_t	size;
	uint32_t	first_page_write = 0;
	uint8_t		*temp_data;
	uint32_t	page_addr = 0;

	if(copy_len < BLOCK_SIZE_128K)
	{
		memset(read_buffer, 0xFF, BLOCK_SIZE_128K);

		/* as we are not checking for read disturbance here no need to pass a separate pointer for read disturbance buffer */
		nand_read_block(blockbase, read_buffer, BLOCK_SIZE_128K);
	...
	page_addr = blockbase;
	for(size = BLOCK_SIZE_128K; size > 0; size -= page_size)
		{
		 /* we need to erase the block for first page only */
		if(first_page_write == 0)
		{
			return_val = nand_erase_block(blockbase);
			if(return_val != 0)
			{
				break;
			}
		...
		return_val = nand_write_page((volatile uint8_t*)page_addr, temp_data, page_size);
		if(return_val != 0)
		{
			break;
		}
...
static void nand_mark_block_bad ( uint32_t blk_addrs)
{
	uint32_t			rval;
	volatile uint32_t	temp;

	blk_addrs = blk_addrs & ~(BLOCK_SIZE_128K-1);

	/* first read the spare area*/
	rval = nand_do_cmd(BCHP_NAND_CMD_START_OPCODE_PAGE_READ, blk_addrs);
	...
	nand_do_cmd(BCHP_NAND_CMD_START_OPCODE_PROGRAM_SPARE_AREA, blk_addrs);
...
static int nand_get_block_status(uint32_t blockbase, int blk_size, int pg_size)
{
	...
	blockbase	&= ~(blk_size - 1);

	rval = nand_do_cmd(BCHP_NAND_CMD_START_OPCODE_PAGE_READ, blockbase);

	if(pg_size >= 2*1024)
	{
		temp = brcmnand_ctrl_read(BCHP_NAND_SPARE_AREA_READ_OFS_0);
		GET_REG_FIELD(temp, BCHP_NAND, SPARE_AREA_READ_OFS_0, BYTE_OFS_0);
	}
	else
	{
		temp = brcmnand_ctrl_read(BCHP_NAND_SPARE_AREA_READ_OFS_4);
		GET_REG_FIELD(temp, BCHP_NAND, SPARE_AREA_READ_OFS_4, BYTE_OFS_5);
	}

	if(temp != 0xFF)
	{
		rval = NAND_BLOCK_STATUS_BAD;
	}
	else
		{
		rval = NAND_BLOCK_STATUS_GOOD;
		}

	return rval;
	}

static uint32_t nand_findLastBMT(uint32_t blockbase, unsigned char *read_buf_ptr)
{
	uint32_t		offset = 0;
	int				status = 0;
	uint32_t		last_table_offset = BMT_TABLE_OFFSET_INVALID;
	unsigned char	block_tag[BMT_TABLE_SIZE];
	unsigned short	BMT_stored_crc = 0;
	unsigned short	BMT_calculated_crc = 0;
	unsigned char	block_erase_flag = 0;

	for(offset = 0; offset<BLOCK_SIZE_128K; offset+=BMT_TABLE_SIZE)
	{
		status = nand_read_block(blockbase+offset, block_tag, BMT_TABLE_SIZE);
		if(status == NAND_ERROR_READ_DISTURB)
		{
			nand_read_block(blockbase, read_buf_ptr, BLOCK_SIZE_128K);

			/* no need to check read status here as we are going to rewrite this block */
			status = nand_block_write(blockbase, read_buf_ptr, 0, 0, NAND_PAGE_SIZE, BLOCK_SIZE_128K);
			if(status != 0)
	{
				printk(KERN_ERR "nand_findLastBMT() - Can't rewrite BMT for fusing!!\n");
				nand_mark_block_bad(blockbase);
				return BMT_TABLE_OFFSET_INVALID;
			}
		}
		else if(status == NAND_ERROR_READ_DISTURB_WITH_UCE)
		{
			last_table_offset	= BMT_TABLE_OFFSET_INVALID;
			block_erase_flag	= 1;
			continue;
}

		BMT_stored_crc		= (block_tag[0] << 8) | block_tag[1];
		BMT_calculated_crc	= GetCRC16(block_tag + BMT_CRC_SIZE, BMT_TABLE_SIZE - BMT_CRC_SIZE);
		//printk(KERN_INFO "=====> CRC of BMT 0 : 0x%04X VS. 0x%04X\n", BMT_stored_crc, BMT_calculated_crc);
	...
	if(block_erase_flag)
	{
		printk(KERN_INFO "nand_findLastBMT() - Block is broken => ERASE BLOCK!!\n");
		nand_erase_block(blockbase);
	}

	printk(KERN_INFO "nand_findLastBMT() - last_table_offset : 0x%08X\n", last_table_offset);

	return last_table_offset;
}

int brcmnand_call_blockmap(struct mtd_info *mtd)
{
	bool		bBMTforFursingFound = FALSE;
	int			status = 0;
	uint8_t			*buf_ptr = 0;
	uint8_t		*read_buf_ptr = 0;
	int				i = 0, tag = 0;
	...
	read_buf_ptr = vmalloc(BLOCK_SIZE_128K);
	if (!read_buf_ptr)
	{
		printk (KERN_ERR "%s: Out of memory size=%dx\n", __FUNCTION__, BLOCK_SIZE_128K);
		return -ENOMEM;
	}

	buf_ptr = vmalloc(BMT_TABLE_SIZE);
	if (!buf_ptr)
	{
		printk (KERN_ERR "%s: Out of memory size=%dx\n", __FUNCTION__, BMT_TABLE_SIZE);
		vfree (read_buf_ptr);
		return -ENOMEM;
	}

	// Block Map for Image Gang
	memset(buf_ptr, 0xFF, BMT_TABLE_SIZE);
	block_offset = BMT_FOR_GANG_OFFSET;
	if(nand_get_block_status(block_offset, BLOCK_SIZE_128K, NAND_PAGE_SIZE) == NAND_BLOCK_STATUS_GOOD)
	{
		status = nand_read_block(block_offset, buf_ptr, BMT_TABLE_SIZE);
		if(status == NAND_ERROR_READ_DISTURB)
		{
			nand_read_block(block_offset, read_buf_ptr, BLOCK_SIZE_128K);

			/* no need to check read status here as we are going to rewrite this block */
			status = nand_block_write(block_offset, read_buf_ptr, 0, 0, NAND_PAGE_SIZE, 0);
			if(status != 0)
			{
				printk(KERN_ERR "brcmnand_call_blockmap() - Can't rewrite BMT for gang!!\n");
				vfree(read_buf_ptr);
				vfree(buf_ptr);
				return -1;
			}
		...
		Calculated_CRC	= GetCRC16(buf_ptr+BMT_CRC_SIZE, BMT_TABLE_SIZE-BMT_CRC_SIZE);
	...
	{
			printk(KERN_ERR "[brcmnand_call_blockmap] CRC not matched - Calculated : 0x%04X VS. Readed : 0x%04X\n", Calculated_CRC, Readed_CRC);
			tag = 0;
		}
	}
	else
	{
		printk(KERN_ERR "[brcmnand_call_blockmap] BMT For gang Broken!!\n");
		vfree (read_buf_ptr);
		vfree (buf_ptr);
		return -EINIT;
	}

	blocknum	= (MIDDLE_OFFSET-RESERVED_AREA) / BLOCK_SIZE_128K;
	memset(blockmap_for_gang, 0xFF, sizeof(blockmap_for_gang));
	if(tag == 1)
	{
		for(i=0;i<blocknum;i++)
			blockmap_for_gang[i] = (buf_ptr[i*2]<<8) + buf_ptr[i*2+1];

		blockmap_for_gang_enable = 1;
	}
	else
	{
		printk(KERN_ERR "[brcmnand_call_blockmap] none of Mapping data\n");
		vfree(read_buf_ptr);
		vfree(buf_ptr);
		return -EINIT;
	}
	...
	for(i= 0; i<blocknum; i++)
	{
		if((blockmap_for_gang[i]%128) == 0)
			printk(KERN_INFO "## Block[%d] is maped at Block[%d]\n", i, blockmap_for_gang[i]);
		if(blockmap_for_gang[i] >= (blocknum-1))
			break;
	}
#endif

	// Block Map for Image Download
	memset(buf_ptr, 0xFF, BMT_TABLE_SIZE);
	block_offset = BMT_FOR_FUSING_OFFSET;
	if(nand_get_block_status(block_offset, BLOCK_SIZE_128K, NAND_PAGE_SIZE) == NAND_BLOCK_STATUS_GOOD)
	{
		last_block_offset = nand_findLastBMT(block_offset, read_buf_ptr);
		if(last_block_offset == BMT_TABLE_OFFSET_INVALID)
	{
			printk(KERN_ERR "[brcmnand_call_blockmap] Table not founded in A\n");
		}
		else
		{
			nand_read_block(block_offset+last_block_offset, buf_ptr, BMT_TABLE_SIZE);
			bBMTforFursingFound = TRUE;
		}
	}
	else
	{
		printk(KERN_ERR "[brcmnand_call_blockmap] BLOCK 1 is BAD\n");
	}

	if(!bBMTforFursingFound)
	{
		...
		if(nand_get_block_status(block_offset, BLOCK_SIZE_128K, NAND_PAGE_SIZE) == NAND_BLOCK_STATUS_GOOD)
	{
			...
			if(last_block_offset == BMT_TABLE_OFFSET_INVALID)
			{
				printk(KERN_ERR "[brcmnand_call_blockmap] Table not founded in B\n");
			}
			else
		{
				nand_read_block(block_offset+last_block_offset, buf_ptr, BMT_TABLE_SIZE);
				...
				//Recovering BMT table in A
				block_offset = BMT_FOR_FUSING_OFFSET;
				if(nand_get_block_status(block_offset, BLOCK_SIZE_128K, NAND_PAGE_SIZE) == NAND_BLOCK_STATUS_GOOD)
			{
					if(nand_erase_block(block_offset) != NAND_BLOCK_STATUS_GOOD)
					{
						printk(KERN_ERR "brcmnand_call_blockmap() - NAND Fursing MAP recovery Erase Fail\n");
						nand_mark_block_bad (block_offset);
					}
					else
					{
						if(nand_block_write(block_offset, buf_ptr, read_buf_ptr, 0, NAND_PAGE_SIZE, BMT_TABLE_SIZE) != NAND_BLOCK_STATUS_GOOD)
			{
							printk(KERN_ERR "brcmnand_call_blockmap() - NAND Fursing MAP recovery Write Fail\n");
							nand_mark_block_bad (block_offset);
						}
					}
			}
	...
	{
			printk(KERN_ERR "[brcmnand_call_blockmap] BLOCK 2 is BAD\n");
		}
	}

	if(bBMTforFursingFound)
	{
		blocknum	= (MIDDLE_OFFSET-RESERVED_BLOCK) / BLOCK_SIZE_128K;
		memset(blockmap_for_fusing, 0xFF, sizeof(blockmap_for_fusing));

		for(i=0;i<blocknum;i++)
			blockmap_for_fusing[i] = (buf_ptr[i*2]<<8) + buf_ptr[i*2+1];

		blockmap_for_fusing_enable = 1;
	}
	else
	{
		printk(KERN_ERR "[brcmnand_call_blockmap] none of Mapping data\n");
		vfree(read_buf_ptr);
		vfree (buf_ptr);
		nand_erase_block(BMT_FOR_GANG_OFFSET);		//To remake BMT table in BSL
		return -EINIT;
	}
	...
	for(i= 0; i<blocknum; i++)
	{
		if((blockmap_for_fusing[i]%128) == 0)
			printk(KERN_INFO "## Block[%d] is maped at Block[%d]\n", i, blockmap_for_fusing[i]);
		if(blockmap_for_fusing[i] >= (blocknum-1))
			break;
	}
#endif
...
#ifdef HUMAX_GENKERNEL
	err = brcmnand_call_blockmap(mtd);
	if(err != 0)
	{
		printk(KERN_ERR "%s: HUMAX NAND BMT Create Fail!!\n", __FUNCTION__);
		return err;
	}
#endif
