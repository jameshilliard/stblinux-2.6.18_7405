...
#ifdef CONFIG_MIPS_BCM7325BX
#define NAND_CHIP_SELECT 		3

#define BLOCK_SIZE_128K			(128*1024)
#define NANDFLASH_SIZE			(128*8*BLOCK_SIZE_128K) // 128Mbyte
#define NAND_CACHE_SIZE			512
#define NAND_BLOCKNUM			(NANDFLASH_SIZE/BLOCK_SIZE_128K)
...
#define BMTPART_SIZE			(3*BLOCK_SIZE_128K) // Block Management Table Size
// Linux Kernel Size
#define KERNEL_SIZE 			(5*8*BLOCK_SIZE_128K) // 5Mbyte
// RomFS (Jffs2)
#define ROMFS_SIZE				(19*8*BLOCK_SIZE_128K) // 19Mbyte

// Reserved BACKUP AREA (For Data Replace)
#define RESERVED_REPLACEMENT	(2*8*BLOCK_SIZE_128K)
	// Reserved AREA (This reserved block need to make BMT for gang)
	#define RESERVED_AREA			(5*BLOCK_SIZE_128K)
#define RESERVED_BLOCK			(RESERVED_AREA+RESERVED_REPLACEMENT)

// Store Area (yaffs2)
#define FREESPACE_SIZE			(97*8*BLOCK_SIZE_128K) // 97Mbyte
// For Linux BBT
#define LINUX_BBT				(4*8*BLOCK_SIZE_128K) // 4Mbyte

	#define RESERVED_AREA_OFFSET	(BMTPART_SIZE+KERNEL_SIZE+ROMFS_SIZE)
	#define MIDDLE_OFFSET			(BMTPART_SIZE+KERNEL_SIZE+ROMFS_SIZE+RESERVED_REPLACEMENT+RESERVED_AREA)
#else
#define NAND_CHIP_SELECT 		1

#define BLOCK_SIZE_128K			(128*1024)
#define NANDFLASH_SIZE			(256*8*BLOCK_SIZE_128K) // 256Mbyte
#define NAND_CACHE_SIZE			512
#define NAND_BLOCKNUM			(NANDFLASH_SIZE/BLOCK_SIZE_128K)
...
#define BMTforGang_SIZE 		BLOCK_SIZE_128K // Block Management Table For Gang
#define BMTforFusing_SIZE		(2*BLOCK_SIZE_128K) // Block Management Table For Fusing
#define BMTPART_SIZE			(BMTforGang_SIZE + BMTforFusing_SIZE)	// Block Management Table Size
// Flash Headerl Size
#define FLASH_HEADER_SIZE 	BLOCK_SIZE_128K				// 128Kbyte
// Linux Kernel Size
#define KERNEL_SIZE 			(3*8*BLOCK_SIZE_128K) // 5Mbyte
// RomFS (Jffs2)
#define ROMFS_SIZE				(27*8*BLOCK_SIZE_128K)					// 60Mbyte

// Reserved BACKUP AREA (For Data Replace)
#define RESERVED_REPLACEMENT	(2*6*BLOCK_SIZE_128K)
// Reserved AREA (This reserved block need to make BMT for gang)
#define RESERVED_AREA			(2*6*BLOCK_SIZE_128K)
#define RESERVED_BLOCK			(RESERVED_AREA+RESERVED_REPLACEMENT)	// 3Mbyte

// For Linux BBT
#define LINUX_BBT				(4*8*BLOCK_SIZE_128K) // 4Mbyte
// Store Area (yaffs2)
#define FREESPACE_SIZE			(NANDFLASH_SIZE - (BMTPART_SIZE + FLASH_HEADER_SIZE + KERNEL_SIZE + ROMFS_SIZE + RESERVED_BLOCK + LINUX_BBT))

#define RESERVED_AREA_OFFSET	(BMTPART_SIZE+FLASH_HEADER_SIZE+KERNEL_SIZE+ROMFS_SIZE)
#define MIDDLE_OFFSET			(RESERVED_AREA_OFFSET+RESERVED_BLOCK)
#endif
