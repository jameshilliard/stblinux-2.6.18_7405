		...
		unsigned long adr, unsigned int chip_op_time )
{
	...
	} while (!map_word_andequal(map, status, OK, OK) && done < usec);
				     ...
				     unsigned long adr, map_word datum, int mode)
{
	...
	int ret=0;
#if defined(HUMAX_FLASH_J3D)
	unsigned long		base_adr = 0;
	map_word			OK = CMD(0x80);
	int 				lock_status, ofs_factor = cfi->interleave * cfi->device_type;
#endif

	adr += chip->start;

#if defined(HUMAX_FLASH_J3D)
	base_adr = adr & ~0x1FFFF;
#endif
	...
	ENABLE_VPP(map);
	/* HUMAX_GENKERNEL(flash unlock) */
#if defined(HUMAX_FLASH_P33)/* bad VPP issue */		
	map_write(map, CMD(0x60), adr);
	map_write(map, CMD(0xD0), adr);	
#elif defined(HUMAX_FLASH_J3D)
	map_write(map, CMD(0x90), base_adr+(2*ofs_factor));
	lock_status = cfi_read_query(map, base_adr+(2*ofs_factor));
	if(lock_status == 0x1)
	{
		printk(KERN_ERR "block status register for 0x%08lx is %x\n", adr, lock_status);
		map_write(map, CMD(0x60), base_adr);
		map_write(map, CMD(0xd0), base_adr);

		do
		{
			cpu_relax();
			status = map_read(map, base_adr);
		} while (!map_word_andequal(map, status, OK, OK));
	}
	map_write(map, CMD(0x70), base_adr);
#endif
...
int cfi_intelext_erase_varsize(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	ofs = instr->addr;
	len = instr->len;
#if defined(HUMAX_FLASH_P33) || defined(HUMAX_FLASH_J3D)
	/* HUMAX_GENKERNEL(flash unlock) */
	cfi_intelext_unlock(mtd, ofs, len);
#endif
				 ...
				 otp_op_t action, int user_regs)
{
	...
	*retlen = 0;
	...
	for ( ; chip_num < cfi->numchips; chip_num += chip_step) {
		...
		field = 0;
#if CONFIG_MTD_OTP
	#ifdef HUMAX_FLASH_J3D
		reg_prot_offset	= 0x159;		//J3D OTP register base offset(BLCR, 0x15A) - 1
		reg_fact_groups	= 0;
		reg_fact_size	= 0;
		reg_user_groups	= 1;
		reg_user_size	= 0x6;			//for 4M (32 Blocks, BLCR(0x2) + OBLA1(0x2) + OBLA2(0x2))
	#else
		#error "OTP Register NOT Defined"
	#endif
#else
