    ...
    IN ULONG arg)
{
   ...
   pTask->kthread_task= kthread_create(fn, (void *)arg, pTask->taskName);
    if (IS_ERR(pTask->kthread_task))
        status = NDIS_STATUS_FAILURE;
    param.sched_priority = WIFI_TASK_PRIO;	
    sched_setscheduler(pTask->kthread_task, SCHED_RR, &param);
    wake_up_process(pTask->kthread_task);
