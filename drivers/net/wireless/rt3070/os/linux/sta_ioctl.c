    ...
    IN  INT                 cmd)
{
    ...
    switch (cmd)
    {
        ...
        case RT_OID_VERSION_INFO:
            ...
            wrq->u.data.length = 12 * sizeof(CHAR);
        ...
        case RT_OID_QUERY_MULTIPLE_CARD_SUPPORT:
            ...
            break;
        case RT_OID_802_11_GET_KEYPRESSED:
        {
            unsigned int data = 0;
            unsigned char pbc_status = 0;
            RTUSBReadMACRegister(pAd, GPIO_CTRL_CFG, &data);
            pbc_status = !(data & 0x00000008);
            wrq->u.data.length = 1;
            Status = copy_to_user(wrq->u.data.pointer, &pbc_status, wrq->u.data.length);
        }
        break;
#ifdef SNMP_SUPPORT
