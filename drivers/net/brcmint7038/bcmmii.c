...
// HUMAX FIX for kernel hang on ethtool
spinlock_t mii_lock = SPIN_LOCK_UNLOCKED;

/* local prototypes */
// HUMAX : mii_getconfig to extern function
...
// HUMAX changed from static to extern
MII_CONFIG mii_getconfig(struct net_device *dev)
{
    BcmEnet_devctrl *pDevCtrl = netdev_priv(dev);
    uint32 val;
