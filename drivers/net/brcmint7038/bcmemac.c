...
// HUMAX patch for kernel hang
	 case ETHTOOL_GSET: /* add command from HUMAX */
	{
		...
		ecmd.phy_address = pDevCtrl->phyAddr;
         	...
         	gConfig = mii_getconfig(dev);

         	if (gConfig & (MII_100MBIT | MII_FULLDUPLEX)) {
             		ecmd.speed = SPEED_100;
             		ecmd.duplex = DUPLEX_FULL;
         	} else if (gConfig & MII_100MBIT) {
             		ecmd.speed = SPEED_100;
             		ecmd.duplex = DUPLEX_HALF;
         	} else if (gConfig & MII_FULLDUPLEX) {
             		ecmd.speed = SPEED_10;
             		ecmd.duplex = DUPLEX_FULL;
         	} else {
             		ecmd.speed = SPEED_10;
    	...
    	case ETHTOOL_GLINK: /* add command from HUMAX */
      {
          struct ethtool_value l_edata;
          
          l_edata.data = bcmIsEnetUp(dev);
     
          if (copy_to_user((void*)useraddr, (void *)&l_edata, sizeof(l_edata)))
            return -EFAULT;
